...
#import <XYTapLabel/XYTapLabel.h>

@interface XYViewController ()<XYTapLabelDelegate>
@property (weak, nonatomic) IBOutlet XYTapLabel *attributeLabel;
...
- (void)viewDidLoad {
    ...
    XYTapLabel * label = [[XYTapLabel alloc] initWithFrame:CGRectMake(0, 200, 200, 0)];
    ...
    [label addTapRange:NSMakeRange(10, 10) with:@"10-20文字"];
    [label addTapRange:NSMakeRange(20, 10) with:@"20-30文字"];
    [label addTapRange:NSMakeRange(30, 10) with:@"30-40文字"];
    [label addTapRange:NSMakeRange(40, 10) with:@"40-50文字"];
    [label addTapRange:NSMakeRange(50, 10) with:@"50-60文字"];
    ...
    self.attributeLabel.text = @"StyleSheet.create这个构造函数并不是必须的，但它提供了一些非常有用的好处。它可以把这些样式值转化为普通的数字id，这些数字id则指向一个内部的样式表，以此来使得样式值变得不可更改（immutable） 和 不可见（opaque）。把样式声明放到文件的末尾还可以确保它们只会在应用中被创建一次，而不是在每次渲染（render方法中）时都被重新创建";
    self.attributeLabel.numberOfLines = 0;
    [self.attributeLabel addTapRange:NSMakeRange(0, 10) with:@"0-10文字"];
    [self.attributeLabel addTapRange:NSMakeRange(10, 10) with:@"10-20文字"];
    [self.attributeLabel addTapRange:NSMakeRange(20, 10) with:@"20-30文字"];
    [self.attributeLabel addTapRange:NSMakeRange(30, 10) with:@"30-40文字"];
    [self.attributeLabel addTapRange:NSMakeRange(40, 10) with:@"40-50文字"];
    [self.attributeLabel addTapRange:NSMakeRange(50, 10) with:@"50-60文字"];
